...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    # EMA  指数移动平均
    # MA也称滑动平均(exponential moving average)，或者叫做指数加权平均(exponentially weighted moving average)，
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            # Warmup
            # 热身训练
            # 在warmp-up阶段，采用的是一维线性插值来进行对每次迭代的学习率进行更新，在warmp-up阶段以后采用的余弦退火算法来对学习率进行更新。
            # cls_loss用于监督类别分类，box_loss用于监督检测框的回归，obj_loss用于监督grid中是否存在物体
            #　用CIOU来表征bbox的回归损失
            if ni <= nw:
                xi = [0, nw]  # x interp
...
def parse_opt(known=False):
    ...
    # 参数进化
    # 超参数进化是一种利用 遗传算法(GA) 进行超参数优化的方法，可以通过该方法选择更加合适自己的超参数。
    ...
    # 这样训练出来的模型鲁棒性强，其可以接受任意大小的图片作为输入，使用尺度小的图片测试速度会快些。
    parser.add_argument('--multi-scale', action='store_true', help='vary img-size +/- 50%%')
    # 是否只训练单类
    parser.add_argument('--single-cls', action='store_true', help='train multi-class data as single-class')
    # 优化器
    # 优化器就是在深度学习反向传播过程中，指引损失函数（目标函数）的各个参数往正确的方向更新合适的大小，
    # 使得更新后的各个参数让损失函数（目标函数）值不断逼近全局最小
    ...
    # 是否使用cos lr  线性学习率  默认False 使用cosine lr
    parser.add_argument('--cos-lr', action='store_true', help='cosine LR scheduler')
    # 是否是标签平滑
    parser.add_argument('--label-smoothing', type=float, default=0.0, help='Label smoothing epsilon')
    # 训练了多少个epoch，如果模型效果未提升，就让模型提前停止训练
    ...
    # 权重层weight，偏执层bais，和BN层
    # 为pg0:BN参数，pg1:一般权重(weight)参数，pg2:偏置(biase)参数，通过param_group来管理3组参数。
    # param_group的意义在于可以针对不同的参数组量身定制学习率(lr)、衰减(weight_decay)、动量(moment)等超参数。
    # 例如在某一模型中，希望特征提取部分的权值参数的学习率小一点，学习更新慢一点，
    # 这时可以把特征提取的参数设置为一组参数，而对于后面全连接分类层，希望其学习率大一点，学习快一点。这时，可以把整个模型参数设置为两组，
    # 一组为特征提取部分的参数，另一部分是全连接层的参数，对这两组设置不同的学习率或超参数。
...
def main(opt, callbacks=Callbacks()):
    ...
    # Evolve hyperparameters (optional)
        ...
        evolve_yaml, evolve_csv = save_dir / 'hyp_evolve_exp4.yaml', save_dir / 'evolve.csv'
        if opt.bucket:
